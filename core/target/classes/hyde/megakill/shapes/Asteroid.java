...
public class Asteroid extends Shape {
    ...
    public Asteroid(float size, float x, float y) {
        this(size);
        this.pos.x = x;
        this.pos.y = y;
    }
    ...
    private void makeAsteroidGraphics() {
        ...
        for ( Vector2 point : points) {
            point.x += Random.getRandomInteger((int)(this.size / 2));
            point.y += Random.getRandomInteger((int)(this.size / 2));
        }
    }

    private void makeCircle(int numberOfPoints) {
        for (double angle=0; angle < 360; angle += 360 / numberOfPoints) {
             float x = (float)Math.sin(Math.toRadians(angle))*size,
                   y = (float)Math.cos(Math.toRadians(angle))*size;
